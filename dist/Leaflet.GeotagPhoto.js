...
var GeotagPhotoCrosshair = L$1.Evented.extend({
  options: {
  ...
  addTo: function addTo(map) {
    ...
    this._element = L$1.DomUtil.create('div', 'leaflet-geotag-photo-crosshair', container);
  ...
  removeFrom: function removeFrom(map) {
    ...
    if (this._element) {
      L$1.DomUtil.remove(this._element);
    }
...
var GeotagPhotoCameraControl = L$3.Control.extend({
  options: {
    position: 'topleft'
  },
  ...
  onAdd: function onAdd(map) {
    ...
    var container = L$3.DomUtil.create('div', controlName + ' leaflet-bar');
  ...
  _createButton: function _createButton(html, title, className, container, fn) {
    var link = L$3.DomUtil.create('a', className, container);
    link.innerHTML = html;
    link.href = '#';
    link.title = title;
    ...
    L$3.DomEvent.on(link, 'mousedown dblclick', L$3.DomEvent.stopPropagation).on(link, 'click', L$3.DomEvent.stop).on(link, 'click', fn, this).on(link, 'click', this._refocusOnMap, this);

    return link;
  },

  onRemove: function onRemove(map) {
    L$3.DomUtil.remove(this._element);
...
var L$2 = window.L;
L$2.geotagPhotoCameraControl = function (geotagPhotoCamera, options) {
  return new GeotagPhotoCameraControl(geotagPhotoCamera, options);
};

var GeotagPhotoCamera = L$2.FeatureGroup.extend({
  ...
  initialize: function initialize(feature, options) {
    ...
    L$2.LayerGroup.prototype.initialize.call(this, layers);
  ...
  _createLayers: function _createLayers() {
    ...
    this._polyline = L$2.polyline(pointList, this.options.outlineStyle);
    this._polygon = L$2.polygon(pointList, Object.assign(this.options.fillStyle, {
      className: 'field-of-view'
    }));

    this._control = L$2.geotagPhotoCameraControl(this, {
      cameraImg: this.options.controlCameraImg,
      crosshairImg: this.options.controlCrosshairImg
    });

    this._cameraMarker = L$2.marker(cameraLatLng, {
    ...
    this._targetMarker = L$2.marker(targetLatLng, {
    ...
    this._angleMarker = L$2.marker(angleLatLng, {
    ...
    var markerSetPos = function markerSetPos(pos) {
      var protoMarkerSetPos = L$2.Marker.prototype._setPos;
  ...
  addTo: function addTo(map) {
    ...
    L$2.FeatureGroup.prototype.addTo.call(this, map);
  ...
  removeFrom: function removeFrom(map) {
    ...
    L$2.FeatureGroup.prototype.removeFrom.call(this, map);
  ...
  _addRotateTransform: function _addRotateTransform(element, rotation) {
    ...
    var transform = element.style[L$2.DomUtil.TRANSFORM];
    var rotate = 'rotate(' + rotation + ')';

    element.style.transformOrigin = 'center center';

    if (transform.indexOf('rotate') !== -1) {
      element.style[L$2.DomUtil.TRANSFORM] = transform.replace(/rotate\(.*?\)/, rotate);
    } else {
      element.style[L$2.DomUtil.TRANSFORM] = transform + ' ' + rotate;
    }
  },
  ...
  _onMarkerKeyDown: function _onMarkerKeyDown(marker, evt) {
    ...
    if (evt.keyCode === 37) {
      // left
      this._moveMarker(marker, L$2.point(-moveDelta, 0));
    } else if (evt.keyCode === 38) {
      // up
      this._moveMarker(marker, L$2.point(0, -moveDelta));
  ...
  getCenter: function getCenter() {
    ...
    return L$2.latLngBounds([this.getCameraLatLng(), this.getTargetLatLng()]).getCenter();
  },
  ...
  getBounds: function getBounds() {
    ...
    var pointList = this._getPointList(this._fieldOfView);
    return L$2.latLngBounds(pointList);
  },
